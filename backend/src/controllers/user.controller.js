...
export const getHomePage = async (req, res) => {
  try {
    ...
    if (!userInfo)
      return res.status(400).json({error: 'User income info not found'});
...
export const getRecentExpenses = async (req, res) => {
  try {
    ...
    if (!allExpenses) {
      return res
        .status(400)
        .json({message: 'Cant get allExpenses in getRecentExpenses'});
    }
...
export const getTotalExpense = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({error: `Error calculating total expenses: ${error.message}`});
  }
};
