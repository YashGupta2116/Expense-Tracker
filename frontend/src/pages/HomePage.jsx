...
const HomePage = () => {
  ...
  const {
    isFetchingGroupedExpense,
    fetchGroupedExpense,
    getTotalExpense,
    isGettingTotalExpense,
  } = useExpenseStore();
  ...
  useEffect(() => {
    const fetchDashboard = async () => {
      ...
      setDashboardInfo({
        ...
        monthlyExpense: totalExpense.totalAmount,
      });
    };
          ...
          {isFetchingGroupedExpense ? (
            ...
            <ul>
              {expenseBreakdown.map((expense, index) => (
                <li
                  key={index}
                  className='flex justify-between text-gray-700 font-bold text-xl'
                >
